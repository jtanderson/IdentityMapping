...
for(var i=1;i<6;i++){
  ...
  for(var j=i+1;j<6;j++){
    ...
    for(var k = j+1; k<6; ++k){
      ...
      var int_ijk = c_i.intersect(c_j_k, {insert: false});
	...
	var int_ijkl = c_i.intersect(c_j_k_l, {insert: false});
	...
	for(var m = l+1; m < 6; m++){
	 ...
	 console.log("Checking intersection of " + i + " " + j + " " + k + " " + l + " " + m); 
	 
	 var c_m = project
	 .getItem({data: {layerName: "circles"}})
	 .getItem({data: {circleId: m}});
	 
	 var int_ijklm = c_i.intersect(c_j_k_l_m, {insert: false});
	 int_ijklm.data.id = ""+i+j+k+l+m;
	 intersectionLayer.addChild(int_ijklm);
	 console.log(int_ijklm);
	 
	}//end of four intersection
      }//end of third intersection
    }//end of two intersection
  }//end of single intersection
...
//gmb
//function for deleting intersections, need to be more complicated???
var a;
function removeIntersection(a){
...
function onMouseDrag(event) {
 ...
 // removeIntersection(int_ij);
  //repeat with further intersections
...
function onMouseUp(event){
  ...
  if (dragged) { 
    ...
    //TODO: Compare w regular remove, why was this code not working before?
