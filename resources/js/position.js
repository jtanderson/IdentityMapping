...
saveIntersect = function (circleID) {
    ...
    for (var i in iLayer.children) {
        ...
        if (
            !iLayer.children[i].isEmpty() &&
            iLayer.children[i].data.intersectId.includes(circleID)
        ) {
...
paper.tool.onMouseDrag = function (event) {
    if (activeItem == null) {
        ...
        if (cLayer.isAncestor(activeItem)) {
            ...
            if (
                handle &&
                (handle.type == "stroke" || handle.type == "segment")
            ) {
                ...
                console.log(
                    "Circle " +
                        activeItem.data.circleId +
                        " has radius " +
                        activeItem.bounds.width / 2
                );
            }
