...
function intersections() {
    ...
    var c_m = project
        .getItem({ data: { layerName: "circles" } })
        .getItem({ data: { circleId: 5 } });
    var two = project
        .getItem({ data: { layerName: "circles" } })
        .getItem({ data: { circleId: 2 } });
    var three = project
        .getItem({ data: { layerName: "circles" } })
        .getItem({ data: { circleId: 3 } });
    var four = project
        .getItem({ data: { layerName: "circles" } })
        .getItem({ data: { circleId: 4 } });
    var one = project
        .getItem({ data: { layerName: "circles" } })
        .getItem({ data: { circleId: 1 } });
    ...
    if (
        ...
        var int_ijklm = iLayer
            .getItem({ data: { intersectId: "1234" } })
            .intersect(c_m, { insert: false }); //5 way int
...
var initialize = function() {
    ...
    for (var i = 1; i <= 5; i++) {
        ...
        var circle_x = document.getElementById(
            "circle-" + circleID + "-center-x"
        ).value;
        var circle_y = document.getElementById(
            "circle-" + circleID + "-center-y"
        ).value;
        var radius = document.getElementById("circle-" + circleID + "-radius")
            .value;
        var line_style = document.getElementById(
            "circle-" + circleID + "-line_style"
        ).value;
        ...
        circle[i]["dbid"] = dbid;
        if (line_style !== "") {
            circle[i]["line_style"] = line_style.split(",").map(function(x) {
                return parseInt(x);
            });
        } else {
            circle[i]["line_style"] = [];
    ...
    for (var i = 0; i < intersectionIdElements.length; i++) {
        ...
        iLayer.getItem({
            data: { intersectId: intPaperId }
        }).fillColor = colorStr;

        var dashStr = document.getElementById("int-" + intId + "-line_style")
            .value;
        if (dashStr) {
            iLayer.getItem({
                data: { intersectId: intPaperId }
            }).dashArray = dashStr.split(",");
        }
    }
}; //end of init
...
paper.tool.onMouseDown = function(event) {
    ...
    if ((hitResult = iLayer.hitTest(event.point))) {
        ...
        if (debug_mode) {
            console.log(
                "User clicked an intersection with id " +
                    hitResult.item.data.intersectId
            );
        }
...
saveIntersect = function(circleID) {
    ...
    for (var i = 0; i < iLayer.children.length; i++) {
        ...
        console.log(
            "This is iLayer.children id " + iLayer.children[i].data.intersectId
        );
