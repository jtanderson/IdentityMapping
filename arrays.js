...
var circlesArray = [undefined, c1, c2, c3, c4, c5];
...
for (var i in intersections){
  for(var j in circlesArray){
    intersections[i].moveAbove(circlesArray[j]);
  }
  for(var k in textArray){
    intersections[i].moveBelow(textArray[k]);
  }
 /*
    ...
    for (var _int in intersections){
      ...
      if( ! intersections[_int].isEmpty() ){
       // intersections[_int].fillColor = "blue";
        ...
        text4.bringToFront();
        text5.bringToFront();
        //can put down when not intersecting but when intersecting cant drop circle
      /*  for(var m in circlesArray){
          circlesArray[m].bringToBack();
        }*/
