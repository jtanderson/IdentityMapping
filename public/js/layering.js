...
console.log(Layer); //creating an array to be stores in local storage later
...
if (answer == "true") {
  ...
  for (var i = 1; i <= 5; i++) {
    //loop for circle creation (random)
  ...
  textLayer.data.layerName = "text"; //confused, difference between point and position?
  ...
  for (var i = 1; i <= 5; i++) {
    //loops the five text creation and binds to the circle objects position
    var c = project.getItem({
      data: {
        circleId: i
      }
    });
...
function creation() {
  var iLayer = project.getItem({
    data: {
      layerName: "intersections"
    }
  });
  var cLayer = project.getItem({
    data: {
      layerName: "circles"
    }
  });
  var tLayer = project.getItem({
    data: {
      layerName: "text"
    }
  });
  ...
  for (var i = 1; i < 6; i++) {
    ...
    if (c_i.visible) {
      for (var j = i + 1; j < 6; j++) {
        ...
        if (c_j.visible) {
          //if two circles overlap, then create intersection
          var int_ij = c_i.intersect(c_j, {
            insert: false
          }); //2 way int
...
var fixLayers = function fixLayers() {
  var iLayer = project.getItem({
    data: {
      layerName: "intersections"
    }
...
var activeItem;
var handle;
var dragged = false; // TODO: logic around this is wrong, it never gets reset
...
function onMouseDown(event) {
  ...
  if (hitResult = iLayer.hitTest(event.point)) {
    ...
    dbarray.push(paper.project.exportJSON()); //creates circles handle
...
function onMouseDrag(event) {
  if (activeItem == null) {
    ...
    if (tester && !intersect) {
      //if it is a circle boundary being hit
      ...
      if (handle && (handle.type == 'stroke' || handle.type == 'segment')) {
        ...
        if (debug_mode) {
          ...
          if (activeItem) {
            ...
            }).translate(event.delta); // + activeItem.position);
...
function onMouseUp(event) {
  ...
  if (dragged) {
    //if the user dragged the circle
    intersections();
...
doSubmit = function doSubmit(e) {
  ...
  var obj = project //.getItem({data: {layerName: "circles"}})
  .getItem({
    data: {
      circleId: parseInt(targetName)
    }
  });
  ...
  var objText = project //.getItem({data: {layerName: "circles"}})
  .getItem({
    data: {
      textId: parseInt(targetName)
    }
  });
  obj.visible = true;
  ...
  insert = true;
  intersections();
  dbarray.push(paper.project.exportJSON()); // console.log(dbarray);
...
module.exports = __webpack_require__(/*! /app/resources/js/layering.js */"./resources/js/layering.js");
